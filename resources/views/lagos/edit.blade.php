        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            @if (!$lago->esActivo())
                <x-alert type="warning"
                    message="Este lago está inactivo. Si deseas reactivarlo, cambia su estado a Activo." />
            @endif

            <x-alert type="info"
                :message="'Editando: ' . $lago->nombre . ' — Registrado el ' . $lago->created_at->format('d/m/Y')" />

            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('lagos.update', $lago) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <x-text-input id="nombre" class="block mt-1 w-full" type="text" name="nombre"
                                :value="old('nombre', $lago->nombre)" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="ubicacion" class="block mt-1 w-full" type="text" name="ubicacion"
                                :value="old('ubicacion', $lago->ubicacion)" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="profundidad" class="block mt-1 w-full" type="number" step="0.01" min="0" name="profundidad"
                                    :value="old('profundidad', $lago->profundidad)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="capacidad_maxima_peces" class="block mt-1 w-full" type="number" min="0" name="capacidad_maxima_peces"
                                    :value="old('capacidad_maxima_peces', $lago->capacidad_maxima_peces)" />
                        ...
                        <div>
                            ...
                            <select id="estado" name="estado" required
                                ...
                                <option value="activo" {{ old('estado', $lago->estado) == 'activo' ? 'selected' : '' }}>Activo</option>
                                <option value="inactivo" {{ old('estado', $lago->estado) == 'inactivo' ? 'selected' : '' }}>Inactivo</option>
                            </select>
                        ...
                        <div>
                            ...
                            <textarea id="observaciones" name="observaciones" rows="4"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-cyan-500 focus:ring-cyan-500">{{ old('observaciones', $lago->observaciones) }}</textarea>
